...
struct SLineInfo
{
	...
	int GetY(int x)
	{
		...
		if(sin(fRadian) != 0)
			nRet = (int)((nDist*1.0 - x*cos(fRadian))/sin(fRadian));
	...
	int GetX(int y)
	{
		...
		if(cos(fRadian) != 0)
			nRet = (int)((nDist*1.0 - y*sin(fRadian))/cos(fRadian));
...
public:
	...
	***************************************************/
	...
	void AdpMedianFilter(CImgProcess *pTo);
	void AddGaussian(CImgProcess *pTo);
	void AddSalt_Pepper(CImgProcess *pTo);
	void AddRayleigh(CImgProcess *pTo);
	void AddUniform(CImgProcess *pTo);
	BOOL FreqInvTuihua(double * pdFilter);
	BOOL FreqInvFilter(double * pdFilter, int nRad);
	BOOL FreqWienerFilter(double * pdFilter, double K);
	...
	void ZigZag(int *in, int *out);
	void iZigZag(int *in, int *out);
	void RunLen(int *in, char *out, int &n);
	void iRunLen(char *in, int n, int *out);
	void iDctCodeBlock(FILE *p, double *dOut );
	int DctCodeBlock(double *dIn, FILE *p);
	int dct_quan_runlrn(FILE *p);
	double idct_quan_runlrn(CImgProcess *pTo, FILE *p);
